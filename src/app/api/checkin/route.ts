...
export async function GET(req: NextRequest) {
  ...
  function mapGoal(g: any) {
    return { id: g.id, title: g.title, frequency: g.frequency, completed: g.completions?.[0]?.completed ?? false, currentStreak: g.currentStreak, reminderTime: g.reminderTime };
  }

  function calcScore(goals: any[]) {
    const t = goals.length;
    const c = goals.filter(g => g.completions?.[0]?.completed).length;
    return { score: t > 0 ? Math.round((c / t) * 100) : 0, completed: c, total: t };
  }

  const sectionData = sections.map(s => {
    const sc = calcScore(s.goals);
    return { id: s.id, name: s.name, goals: s.goals.map(mapGoal), ...sc };
  });
  ...
  return NextResponse.json({
    ...
    unsectioned: unsectioned.map(mapGoal),
    unsectionedScore: unsc,
    consolidatedScore: overall.score,
...
export async function POST(req: NextRequest) {
  ...
  await prisma.goalCompletion.upsert({
    ...
    create: { goalId, date, completed: completed ?? true, dailyLogId: dailyLog.id },
  ...
  if (goal && completed) {
    ...
    const prevCompletion = await prisma.goalCompletion.findUnique({ where: { goalId_date: { goalId, date: yStr } } });
    const newStreak = (prevCompletion?.completed) ? goal.currentStreak + 1 : 1;
    await prisma.goal.update({ where: { id: goalId }, data: { currentStreak: newStreak, bestStreak: Math.max(goal.bestStreak, newStreak) } });
  } else if (goal && !completed) {
    await prisma.goal.update({ where: { id: goalId }, data: { currentStreak: 0 } });
  }

  // Recalculate consolidated score
  const allGoals = await prisma.goal.findMany({
    where: { userId: session.user.id, isActive: true },
    include: { completions: { where: { date, completed: true } } },
  });
  const totalG = allGoals.length;
  const doneG = allGoals.filter(g => g.completions.length > 0).length;
  ...
  const sections = await prisma.goalSection.findMany({
    where: { userId: session.user.id, isActive: true },
    include: { goals: { where: { isActive: true }, include: { completions: { where: { date, completed: true } } } } },
  });
  const sectionScores = sections.map(s => {
    const t = s.goals.length; const c = s.goals.filter(g => g.completions.length > 0).length;
    return { id: s.id, score: t > 0 ? Math.round((c / t) * 100) : 0, completed: c, total: t };
  });
  ...
  for (const f of ["daily", "weekly", "monthly"]) {
    const fg = allGoals.filter(g => g.frequency === f);
    const fc = fg.filter(g => g.completions.length > 0).length;
